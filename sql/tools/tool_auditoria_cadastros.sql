...
SELECT ca.nu_ine,
       ...
       COUNT(CASE WHEN ca.dt_atualizacao >= NOW() - INTERVAL '24 months' THEN 1 END) AS cadastros_vigentes_24m,
       COUNT(CASE WHEN ca.dt_atualizacao < NOW() - INTERVAL '24 months' OR ca.dt_atualizacao IS NULL THEN 1 END) AS cadastros_expirados,
       ROUND(COUNT(CASE WHEN ca.dt_atualizacao >= NOW() - INTERVAL '24 months' THEN 1 END)::numeric / NULLIF(COUNT(ca.co_seq_cidadao), 0) * 100, 2) AS perc_cadastros_vigentes
