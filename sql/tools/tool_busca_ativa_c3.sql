...
SELECT
  ...
  COALESCE(m.afericoes_pa,0)::int AS afericoes_pa,
  COALESCE(m.antropometrias,0)::int AS registros_peso_altura,
  COALESCE(v.visitas_gestacao,0)::int AS visitas_gestacao,
  COALESCE(f.registros,0)::int AS registros_dtpa,
  (COALESCE(x.hiv_t1,false)::int + COALESCE(x.sifilis_t1,false)::int + COALESCE(x.hepb_t1,false)::int + COALESCE(x.hepc_t1,false)::int)::int AS exames_1_trimestre_cumpridos,
  (COALESCE(x.hiv_t3,false)::int + COALESCE(x.sifilis_t3,false)::int)::int AS exames_3_trimestre_cumpridos,
  COALESCE(q.consultas_puerperio,0)::int + COALESCE(pm.registros,0)::int AS consultas_puerperio,
  COALESCE(v.visitas_puerperio,0)::int AS visitas_puerperio,
  COALESCE(sb.atividades,0)::int AS atividades_saude_bucal,
