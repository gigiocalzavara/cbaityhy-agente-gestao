...
avaliacao AS (
  SELECT p.*,
    COALESCE(q.consultas_gestacao,0)::int AS consultas_prenatal,
    COALESCE(m.afericoes_pa,0)::int AS afericoes_pa_total,
    COALESCE(m.antropometrias,0)::int AS registros_peso_altura_total,
    COALESCE(v.visitas_gestacao,0)::int AS visitas_gestacao_total,
    COALESCE(f.registros,0)::int AS registros_dtpa_total,
    (COALESCE(x.hiv_t1,false)::int + COALESCE(x.sifilis_t1,false)::int + COALESCE(x.hepb_t1,false)::int + COALESCE(x.hepc_t1,false)::int)::int AS exames_t1_total,
    (COALESCE(x.hiv_t3,false)::int + COALESCE(x.sifilis_t3,false)::int)::int AS exames_t3_total,
    (COALESCE(q.consultas_puerperio,0) + COALESCE(pm.registros,0))::int AS consultas_puerperio_total,
    COALESCE(v.visitas_puerperio,0)::int AS visitas_puerperio_total,
    COALESCE(sb.atividades,0)::int AS atividades_saude_bucal_total,
...
SELECT
  ...
  CASE WHEN NOT a.em_puerperio THEN GREATEST(0, FLOOR((CURRENT_DATE - a.inicio_gestacao) / 7.0))::int ELSE NULL END AS idade_gestacional_semanas,
  a.consultas_prenatal,
  a.afericoes_pa_total AS afericoes_pa,
  a.registros_peso_altura_total AS registros_peso_altura,
  a.visitas_gestacao_total AS visitas_gestacao,
  a.registros_dtpa_total AS registros_dtpa,
  a.exames_t1_total AS exames_1_trimestre_cumpridos,
  a.exames_t3_total AS exames_3_trimestre_cumpridos,
  a.consultas_puerperio_total AS consultas_puerperio,
  a.visitas_puerperio_total AS visitas_puerperio,
  a.atividades_saude_bucal_total AS atividades_saude_bucal,
