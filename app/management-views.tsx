  ...
  return <div className="module-page indicator-page-redesign">
    ...
    {selectedResult && <>
      <section className="indicator-summary-grid">
        ...
        <article><span>{isC2Partial ? "E · Vacinação" : selectedIndicator.id === "C3" ? "Pessoas identificadas" : "Atendem ao critério"}</span><strong>{isC2Partial && c2VaccinationScore !== null ? `${c2VaccinationScore.toLocaleString("pt-BR", { maximumFractionDigits: 2 })}%` : pretty(numerator)}</strong><small>{isC2Partial ? `${pretty(numerator)} crianças com esquema identificado` : "Numerador municipal"}</small></article>
        <article><span>{isC2Partial ? "Crianças no recorte local" : "População considerada"}</span><strong>{pretty(denominator)}</strong><small>{isC2Partial ? "Denominador do recorte vacinal; não é o denominador oficial integral do C2" : "Denominador municipal"}</small></article>
      </section>
